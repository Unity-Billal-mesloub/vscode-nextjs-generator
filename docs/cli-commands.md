...
```jsonc
{
  "nextjs.server.turbopack": true,
  "nextjs.server.experimentalHttps": true,
}
```

- `nextjs.server.turbopack`: Turbo mode is a new mode that enables incremental compilation and dramatically improves the startup time of Next.js development servers. It's only available in Next.js 14 or newer.
